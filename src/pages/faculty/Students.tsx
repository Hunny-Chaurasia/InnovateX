...
import { Card, Badge, Button, Avatar, Input, VirtualIdCard } from '../../components/ui';
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 p-4" style={{ background: 'rgba(0,0,0,0.7)' }}>
      <div className="w-full max-w-sm rounded-2xl border p-6" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => name && email && setDone(true)} className="flex-1" disabled={!name || !email}>
...
export default function Students() {
  ...
  const filtered = MOCK_STUDENTS_FACULTY.filter(s =>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b" style={{ borderColor: 'var(--border)' }}>
                {['Student', 'Virtual ID', 'Status', 'Projects', 'Joined', 'Actions'].map(h => (
