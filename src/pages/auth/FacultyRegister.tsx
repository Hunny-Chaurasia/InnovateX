import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { Input, Button, VirtualIdCard } from '../../components/ui';

function validateEduEmail(email: string): string {
  if (!email) return 'Email is required';
  if (!email.includes('@')) return 'Enter a valid email address';
  const domain = email.split('@')[1] || '';
  if (!domain.endsWith('.edu') && !domain.endsWith('.tech') && !domain.endsWith('.ac.in')) {
    return 'Must be an institutional email (.edu, .tech, or .ac.in domain)';
  }
  return '';
}

export default function FacultyRegister() {
  const navigate = useNavigate();
  const [form, setForm] = useState({ email: '', institution: '', designation: '', linkedin: '' });
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [submitted, setSubmitted] = useState(false);
  const generatedId = 'FAC-2024-0041';

  const set = (k: string) => (v: string) => setForm(f => ({ ...f, [k]: v }));

  const handleSubmit = () => {
    const errs: Record<string, string> = {};
    const emailErr = validateEduEmail(form.email);
    if (emailErr) errs.email = emailErr;
    if (!form.institution) errs.institution = 'Institution name is required';
    if (!form.designation) errs.designation = 'Designation is required';
    if (!form.linkedin) errs.linkedin = 'LinkedIn profile URL is required';
    else if (!form.linkedin.includes('linkedin.com')) errs.linkedin = 'Enter a valid LinkedIn URL';
    setErrors(errs);
    if (Object.keys(errs).length === 0) setSubmitted(true);
  };

  if (submitted) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6" style={{ background: 'var(--background)' }}>
        <div className="w-full max-w-sm text-center">
          <div
            className="w-14 h-14 rounded-full flex items-center justify-center text-2xl mx-auto mb-4"
            style={{ background: 'rgba(5,150,105,0.15)', border: '2px solid rgba(5,150,105,0.4)' }}
          >
            ✓
          </div>
          <h2 className="text-2xl font-bold mb-2" style={{ fontFamily: 'DM Sans', color: 'var(--foreground)' }}>
            Registration Submitted
          </h2>
          <p className="text-sm mb-6" style={{ color: 'var(--muted-foreground)' }}>
            Your application is under review. Your Virtual ID has been generated below —
            a setup link has been sent to <strong style={{ color: 'var(--foreground)' }}>{form.email}</strong>.
          </p>
          <div className="flex justify-center mb-6">
            <VirtualIdCard id={generatedId} label="Your Virtual ID" />
          </div>
          <p className="text-xs mb-4" style={{ color: 'var(--muted-foreground)' }}>
            Use this ID to sign in after setting your password via the email link.
          </p>
          <Button onClick={() => navigate('/auth/login')} size="lg" className="w-full">
            Go to Sign In
          </Button>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen flex items-center justify-center p-6" style={{ background: 'var(--background)' }}>
      <div className="w-full max-w-md">
        <button onClick={() => navigate('/')} className="flex items-center gap-1 text-xs mb-6"
          style={{ color: 'var(--muted-foreground)' }}>
          ← Back
        </button>
        <h1 className="text-2xl font-bold mb-1" style={{ fontFamily: 'DM Sans', color: 'var(--foreground)' }}>
          Faculty Registration
        </h1>
        <p className="text-sm mb-6" style={{ color: 'var(--muted-foreground)' }}>
          Register as a faculty mentor on InnovateX. Institutional email required.
        </p>

        <div className="p-6 rounded-2xl border flex flex-col gap-4" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
          <Input
            label="Institutional Email"
            type="email"
            placeholder="you@university.edu"
            value={form.email}
            onChange={set('email')}
            error={errors.email}
            required
          />
          <Input
            label="Institution Name"
            placeholder="Delhi Technological University"
            value={form.institution}
            onChange={set('institution')}
            error={errors.institution}
            required
          />
          <Input
            label="Designation"
            placeholder="Associate Professor, Dept. of CSE"
            value={form.designation}
            onChange={set('designation')}
            error={errors.designation}
            required
          />
          <Input
            label="LinkedIn Profile URL"
            placeholder="https://linkedin.com/in/your-name"
            value={form.linkedin}
            onChange={set('linkedin')}
            error={errors.linkedin}
            required
            prefix={<span style={{ fontSize: 13 }}>in</span>}
          />

          <div className="p-3 rounded-lg text-xs" style={{ background: 'var(--muted)', color: 'var(--muted-foreground)' }}>
            After registration, a <strong style={{ color: 'var(--foreground)' }}>Virtual ID</strong> will be auto-generated and a
            password setup link will be sent to your institutional email.
          </div>

          <Button onClick={handleSubmit} size="lg" className="w-full">
            Submit Registration
          </Button>
        </div>
      </div>
    </div>
  );
}
