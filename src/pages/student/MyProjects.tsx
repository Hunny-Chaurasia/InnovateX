...
import { Card, Badge, StageBadge, InstitutionBadge, Button, Avatar, ProjectJourneyStepper } from '../../components/ui';
...
function MentorModal({ project, onClose }: { project: typeof MOCK_PROJECTS[0]; onClose: () => void }) {
  ...
  const hasSchoolAndCollege = project.team.some(m => m.type === 'School') && project.team.some(m => m.type === 'College');
          ...
          {MOCK_MENTORS.map(m => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs truncate" style={{ color: 'var(--muted-foreground)' }}>{m.domain}</p>
              </div>
...
export default function MyProjects() {
  ...
  const mentorProject = MOCK_PROJECTS.find(p => p.id === mentorModal);
